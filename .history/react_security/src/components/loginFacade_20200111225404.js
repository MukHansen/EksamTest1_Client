...
class LoginFacade {
  ...
  editHobby = (hobby) => {
    ...
    const options = this.makeOptions("PUT", true); //True add's the token
    return fetch(URL + "/api/person/edithobby", options).then(handleHttpErrors);
  };

  createHobby = (hobby) => {
    console.log("Creating hobby", hobby);
    const options = this.makeOptions("POST", true); //True add's the token
    return fetch(URL + "/api/person/createhobby", options).then(handleHttpErrors);
  };
