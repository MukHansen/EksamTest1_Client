...
function Hobbies({ loggedIn, allHobbies, setAllHobbies }) {
    ...
    const deleteHobbyByName = () => {
        if (loggedIn) {
            const getData = async () => {
                try {
                    const deletingHobby = await facade.fetchPersonsByHobby(document.getElementById("hobbyToDelete").value);
    ...
    if(loggedIn){
        ...
        <hr />
        <input placeholder="HobbyName" type="text" id="hobbyToDelete" value=""/>
