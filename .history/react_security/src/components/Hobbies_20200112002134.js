...
function Hobbies({ loggedIn, allHobbies, setAllHobbies }) {
    console.log("Hobbies");
    const [hobby, setHobby] = useState({name:"", description:""});
    ...
    const addEditHobby = () => {
        if (loggedIn) {
            ...
            const getData = async () => {
                try {
                    setHobby(
                        document.getElementById("hobbyToEditName").value,
                        document.getElementById("hobbyToEditDescription").value);
                    const addEditingHobby = 
                    await facade.addEditHobby(hobby);
