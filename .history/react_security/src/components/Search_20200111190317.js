    ...
    const searchByEmail = () => {
      ...
      const searchByHobby = () => {
        if (loggedIn) {
            const getData = async () => {
                try {
                    const hobbyData = await facade.fetchPersonByPhone(document.getElementById("hobbyName").value);
                    console.log("fetchingPersonsByHobby personData", hobbyData);
                    setPersonsList(hobbyData);
                } catch (e) {
                    console.log("err", e);
                }
            };
